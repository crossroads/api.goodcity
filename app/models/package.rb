class Package < ActiveRecord::Base
  ...
  validate :packages_locations_validator
  ...
  def has_packages_locations?
    packages_locations.exists?
  end

  def packages_locations_validator
    if has_packages_locations?
      errors.add(:inventory_number, "should be present") if !inventory_number?
      errors.add(:state, "should be received") if !received?
    end
  end
end
