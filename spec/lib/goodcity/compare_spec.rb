...
#     let(:goodcity_klass) { StockitActivity }
#     let(:stockit_objects) { {"1" => {"id" => "1", "name" => "Name"}} }
...
#     let(:stockit_hash) { {"id" => "1", "box_id" => "2"} }
...
#       expect(subject).to receive(:compare_objects).with(Package, {"1" => stockit_hash}, [:box_id, :case_number, :code_id, :condition, :description, :grade, :height, :inventory_number, :length, :location_id, :pallet_id, :quantity, :sent_on, :width])
...
#       expect(subject.send(:stockit_json, klass, "items")).to eql("1" => stockit_hash)
#     end
