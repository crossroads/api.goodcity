...
describe Message, :type => :model do
  ...
  describe ".on_offer_submittion" do
    let(:message_details) {{body: "I have made an offer.", sender_id: user.id,
          is_private: false, offer_id:offer.id}}
    let(:update_message) { build(:message, message_details) }
    let(:subscription) { create :subscription, {state: 'read', offer_id: offer.id,
      user_id: user.id, message_id: update_message.id} }
