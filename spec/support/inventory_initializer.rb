module InventoryInitializer
  def initialize_inventory(*packages, location: nil)
    [packages].flatten.each do |pkg|
      ...
      create :packages_inventory, package: pkg, quantity: pkg.received_quantity, location: dest_location, action: 'inventory'

      pkg.orders_packages.dispatched.each do |orders_package|
        create :packages_inventory, package: pkg, quantity: - orders_package.quantity, action: 'dispatch', location: dest_location, source: orders_package
      end
