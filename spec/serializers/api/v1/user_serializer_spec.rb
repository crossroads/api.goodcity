...
describe Api::V1::OfferSerializer do
  ...
  let(:charity_user) { create(:user, :charity, :title_en, :title_zh_tw) }
  ...
  context "charity" do
    ...
    it "creates JSON" do
      ...
      expect(charity_json['user']['title_en']).to eql(charity_user.title_en)
      expect(charity_json['user']['title_zh_tw']).to eql(charity_user.title_zh_tw)
