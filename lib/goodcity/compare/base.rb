require 'csv'
require 'goodcity/compare/goodcity_table_loader'
require 'goodcity/compare/stockit_table_loader'
    ...
    class Base
      ...
      def stockit_data
        bm("Running Stockit #{@table} SQL") do
          raw_data = Goodcity::Compare::StockitTableLoader.load("items")
      ...
      def goodcity_data
        bm("Running GoodCity #{@table} SQL") do
          raw_data = Goodcity::Compare::GoodcityTableLoader.load(@table)
          Hash[raw_data.map{|h| [h["stockit_id"], h] }]
        end
      end
