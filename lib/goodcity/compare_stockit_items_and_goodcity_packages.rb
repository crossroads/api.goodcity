...
# Rails Console
#   > require 'goodcity/compare_stockit_items_and_goodcity_packages'
#   > cmp = Goodcity::CompareStockitItemsAndGoodcityPackages.new
#   > cmp.run
...
module Goodcity
  class CompareStockitItemsAndGoodcityPackages
    ...
    def run
      ...
      if (check_file_exists?(GC_FILE_NAME) and check_file_exists?(STOCKIT_FILE_NAME))
        puts("Missing files for comparison")
      else
        process
        report
      end
